...
of numerical scores, $\mathsf{auth}(p)$ and $\mathsf{hub}(p)$.
...
For each page $p$, update $\mathsf{auth}(p)$
...
After a number of iterations, the values $\mathsf{auth}(p)$ and
$\mathsf{hub}(p)$ stabilize, in the sense that further applications of
